...
TOPIC_LABEL = VIDEO_REQUIRED_PHRASES[0] if VIDEO_REQUIRED_PHRASES else "русский язык"
...
REQUIRE_YOUTUBE_VIDEO = os.getenv("REQUIRE_YOUTUBE_VIDEO", "0").strip().lower() in {"1", "true", "yes", "on"}
...
def parse_youtube_api(source: Dict[str, Any]) -> List[PostItem]:
    ...
    for v in details_data.get("items", []):
        ...
        summary = f"Видео по теме «{TOPIC_LABEL}» для 2 класса. Длительность: {mins}:{sec:02d}."
...
def parse_youtube_search(source: Dict[str, Any]) -> List[PostItem]:
    ...
    try:
        ...
        if proc.returncode != 0:
            logger.warning("yt-dlp search returned code %s (will try partial output): %s", proc.returncode, (proc.stderr or proc.stdout)[-500:])
...
def format_message(item: PostItem) -> str:
    ...
    parts.append(f"Коротко: {safe_summary}")
    parts.append(f"Источник: <b>{safe_source}</b>")
    if item.link:
        link_value = encrypt_link(item.link)
        if link_value:
            if LINK_ENCRYPTION_ENABLED:
                parts.append(f"Ссылка (зашифрована): <code>{html.escape(link_value)}</code>")
            else:
                parts.append(f"Ссылка: {html.escape(link_value)}")
    return "\n".join(parts)
    ...
    for item in items:
        ...
        for attempt in range(1, 4):
            try:
                ...
                # Optional strict mode: require video for YouTube items.
                if REQUIRE_YOUTUBE_VIDEO and ("youtube.com/watch" in (item.link or "") or "youtu.be/" in (item.link or "")) and (not video_sent):
